/**
 * tsdown build for dsh-dashboard: the host-half lib (lib/index.js and
 ...
 *   the package-name id `dsh-dashboard` (the client-modules compose
 ...
 *   `dsh-external/dsh-dashboard` (the registry browser-side `arrive()`
 ...
 * differing: the official channel uses the package name (`dsh-dashboard`)
 * and the registry channel uses the manifest id
 * (`dsh-external/dsh-dashboard`).
 ...
 *
 * Chunk css tags use the constant plugin id `dsh-dashboard` (matching
...
function chunkBundle(name: string): UserConfig {
  return {
    ...
    noExternal: (id: string) => (CLIENT_EXTERNALS.includes(id) ? undefined : true),
    plugins: [purityGatePlugin(), makeCssPlugin('dsh-dashboard')],
    outputOptions: {
...
export default [
  ...
  clientBundle('dsh-dashboard', 'client.js'),
  // Plugin-registry channel: bundle id = manifest id (dsh.plugin.json `id`).
  clientBundle('dsh-external/dsh-dashboard', 'client-registry.js'),
