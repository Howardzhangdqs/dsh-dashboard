...
describe('built-in file viewer registrations', () => {
  ...
  it('the code viewer declares the insert-format related setting', () => {
    const { service } = setup()
    const code = service.getFileViewers().find(v => v.id === 'code')
    // The include-text switch is gone: the format string alone decides
    // whether the selection rides along ({selection} placeholder).
    expect(code?.settings?.toggles ?? []).toEqual([])
    const inputs = code?.settings?.inputs ?? []
    expect(inputs.map(i => i.key)).toEqual(['codeSelectionFormat'])
    expect(inputs[0]?.title).toBeDefined()
    // The desc documents every placeholder AND carries usage examples
    // (both halves of the copy exist in every locale's dictionary).
    const desc = inputs[0]?.desc?.()
    for (const token of ['{path}', '{abspath}', '{start}', '{end}', '{lines}', '{selection}', '\\n', '@{path}{lines}']) {
      expect(desc, token).toContain(token)
    }
    expect(inputs[0]?.placeholder).toBeDefined()
  })
