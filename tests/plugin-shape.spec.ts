 ...
 * packages/ui/jsonrpc).
 */
describe('dsh-dashboard plugin export shape', () => {
  it('has the namespace-plugin export shape (no stray default) so the Loader keeps name/inject/Config/apply', () => {
    ...
    expect(unwrapped.name).toBe('dsh-dashboard')
  ...
  it('registers the side card preferences schema with the documented defaults', async () => {
    ...
    expect(SIDEBAR_PREFS_NS).toBe('dsh-dashboard')
    ...
    // The selection popup's insert defaults to the @-reference format.
    expect(resolved.codeSelectionFormat).toBe('@{path}{lines}')
    ...
    expect(overridden).toEqual({ openByDefault: false, defaultWidthPercent: 45, autoOpenSubagent: true, autoOpenJobs: true, agentTerminalTools: false, bottomPanelAutoTerminal: true, interceptOpenPath: true, htmlViewerNoSandbox: false, htmlViewerDefaultUnsafe: false, codeSelectionFormat: '@{path}{lines}', tabsEnabled: {}, viewersEnabled: {} })
  })
})
