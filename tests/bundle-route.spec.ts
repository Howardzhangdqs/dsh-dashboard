...
describe('/sidebar/bundle data assets (icons.json.gz)', () => {
  it('serves the gzip member verbatim with content-encoding gzip + json type', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'bundle-asset-'))
    const gz = gzipSync(JSON.stringify({ file: 'file', icons: { file: '<svg/>' } }))
    writeFileSync(join(dir, 'icons.json.gz'), gz)
    const handler = createBundleRouteHandler(() => true, dir)
    const res = fakeRes()
    await handler(req('GET', '/sidebar/bundle/icons.json.gz'), res as unknown as ServerResponse)
    expect(res.status).toBe(200)
    expect(res.headers['content-type']).toBe('application/json; charset=utf-8')
    expect(res.headers['content-encoding']).toBe('gzip')
    expect(res.headers['cache-control']).toBe('no-cache')
    expect(res.body).toBe(gz.toString())
    rmSync(dir, { recursive: true, force: true })
  })

  it('revalidates with ETag (304) like chunks do', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'bundle-asset-'))
    writeFileSync(join(dir, 'icons.json.gz'), gzipSync('{"a":1}'))
    const handler = createBundleRouteHandler(() => true, dir)
    const first = fakeRes()
    await handler(req('GET', '/sidebar/bundle/icons.json.gz'), first as unknown as ServerResponse)
    const etag = first.headers.etag ?? ''
    expect(typeof etag).toBe('string')
    expect(etag).not.toBe('')
    const revalidate = fakeRes()
    await handler(req('GET', '/sidebar/bundle/icons.json.gz', { 'if-none-match': etag }), revalidate as unknown as ServerResponse)
    expect(revalidate.status).toBe(304)
    rmSync(dir, { recursive: true, force: true })
  })

  it('rejects unknown asset names (allowlist, no traversal)', async () => {
    const handler = createBundleRouteHandler(() => true, mkdtempSync(join(tmpdir(), 'bundle-asset-')))
    const res = fakeRes()
    await handler(req('GET', '/sidebar/bundle/secrets.json.gz'), res as unknown as ServerResponse)
    expect(res.status).toBe(404)
  })

  it('404s when the packed asset is missing (not yet built)', async () => {
    const handler = createBundleRouteHandler(() => true, mkdtempSync(join(tmpdir(), 'bundle-asset-')))
    const res = fakeRes()
    await handler(req('GET', '/sidebar/bundle/icons.json.gz'), res as unknown as ServerResponse)
    expect(res.status).toBe(404)
