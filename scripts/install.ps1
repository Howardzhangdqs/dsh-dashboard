# =============================================================================
# dsh-dashboard 一键安装脚本（官方 CLI 方式，Windows PowerShell 5.1+ / pwsh）
#
# 通过 DSH 官方插件命令安装 npm 包并自动挂载：
#   dsh plugin --profile web add dsh-dashboard@<version>
#
...
#   # 指定版本 / 装完重启
#   & ([scriptblock]::Create((irm 'https://raw.githubusercontent.com/Howardzhangdqs/dsh-dashboard/main/scripts/install.ps1'))) -Version 0.10.2 -Restart
#   # 本地保存后运行
...
if (!/^\s*-\s+dsh-dashboard\s*$/m.test(t)) {
...
if ($bundles -notcontains $PKG) {
  Warn 'dsh-dashboard 未出现在 dsh.profile.bundles 中——挂载未注册。'
