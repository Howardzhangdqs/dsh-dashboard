# dsh-dashboard 插件接入文档

> 面向**消费插件开发者**：如何让你的插件向 dsh-dashboard 注册新的侧边栏页面（tab）和文件类型预览器。

dsh-dashboard（前名 better-sidebar）从 v0.4.0 起暴露 `ctx.dashboard` 服务（Cordis context 属性），其他插件通过 `registerTab` / `registerFileViewer` 注册扩展点，返回 disposer 由 Cordis fiber 自动管理生命周期（HMR-safe）。
...
- **服务名**：`dashboard`（即 `ctx.dashboard`；旧名 `betterSidebar` 作为兼容别名仍然解析，外部插件无需改动）
- **发布侧**：dsh-dashboard 的 client half（`src/client/index.tsx`，通过 `ctx.provide('dashboard', service)` 发布（旧 id `betterSidebar` 同时再发布一次作为兼容别名））
- **消费侧**：你的插件的 client half（`inject = ['dashboard', ...]`，然后 `ctx.dashboard.registerTab(...)`）
- **类型合并**：`declare module 'cordis' { interface Context { dashboard: BetterSidebarService } }` 由 `dsh-dashboard` 包导出；消费插件 `import type {} from 'dsh-dashboard'` 即触发类型合并

> ⚠️ **host 半不发布此服务**：`ctx.dashboard` 只在 client 侧存在。如果你的插件 host 半需要读 dsh-dashboard 状态，走插件自己的 HTTP/WS 路由（`/sidebar/api/*`），不走服务。
...
```jsonc
{
  ...
  "peerDependencies": {
    "cordis": "^4.0.0-rc.7",
    "dsh-dashboard": "workspace:*"
  },
  "peerDependenciesMeta": {
    "dsh-dashboard": { "optional": true }
  }
}
```

- `dsh-dashboard` 必须声明为 **peerDependency**（不是 dependency，避免重复实例化）
- 标记 `optional: true` 让你的插件在 dsh-dashboard 未安装时也能加载（注册代码会因为 `ctx.dashboard` 为 undefined 而跳过）
...
export const inject = ['dashboard', 'slots']  // 声明服务依赖（旧名 'betterSidebar' 别名同样有效）

export function apply(ctx: Context): void {
  // 注册一个 sidebar tab
  ctx.effect(() =>
    ctx.dashboard.registerTab({
      id: 'my-plugin:db',
      title: () => 'Database',
      icon: <DbIcon />,
      order: 50,
  ...
  // 注册一个文件预览器
  ctx.effect(() =>
    ctx.dashboard.registerFileViewer({
      id: 'my-plugin:csv',
      exts: ['csv'],
...
> ⚠️ **构建期纯度门**：client bundle 禁止 value-import 别的插件代码（`tsdown.config.ts` 的纯度门会挡）。`import type {}` 会被擦除，**不触发门禁**——所以类型可以自由共享，运行时符号不行。所有运行时交互必须走 `ctx.dashboard` 的方法调用。

### 2.3 类型导入

```ts
import type { TabDescriptor, FileViewerDescriptor, BetterSidebarService } from 'dsh-dashboard'
```
...
interface TabDescriptor {
  ...
  settings?: {
    toggles?: readonly {
      /** SidebarPrefs 字段名（内置键：'autoOpenSubagent' / 'agentTerminalTools' / 'htmlViewerNoSandbox' / 'htmlViewerDefaultUnsafe'） */
      key: string
      title: string | (() => string)
      desc?: string | (() => string)
    }[]
    /** 字符串输入行（同样在齿轮弹窗中；key 必须是宿主 PrefsSchema 的 string 字段，内置键：'codeSelectionFormat'，默认 '@{path}{lines}'） */
    inputs?: readonly {
      key: string
      title: string | (() => string)
      desc?: string | (() => string)
...
interface TabComponentProps {
  ...
  store: SidebarStore          // dsh-dashboard 的状态 store（可调 reduce 等）
...
// 外部触发打开：
ctx.dashboard.openTab({ type: 'my-plugin:doc', title: 'Spec.md', id: 'doc:spec' })
```
...
interface FileViewerDescriptor {
  ...
  settings?: { toggles?: readonly { key: string; title: string | (() => string); desc?: string | (() => string) }[]; inputs?: readonly { key: string; title: string | (() => string); desc?: string | (() => string); placeholder?: string | (() => string) }[] }
  /** 渲染函数 */
...
interface BetterSidebarService {
   ...
   * 内容型打开（带 path seed）必须落在视野内：承载落点 pane 的面板
...
> **声明式设置（v0.4.1+）**：每个注册的 tab/viewer 自动出现在 DSH 设置页「侧边卡片」分区的清单里——响应式网格中的**小卡片**（图标 + 标题 + 类型 id + **高亮 = 启用**，勾选徽标钉在卡片最右端，viewer 卡片还显示扩展名），开关持久化到 `SidebarPrefs.tabsEnabled / viewersEnabled`（开放 map，缺省 = 启用）。关闭语义：tab 从 `+` 菜单消失、`openTab` 拒绝新开、子代理自动展开 / agent 终端自动补 tab 等派生流程停止，**已打开的 tab 保留**；viewer 被 `matchFileViewer` 跳过，文件落到下一个匹配。`settings.toggles` 声明的布尔开关与 `settings.inputs` 声明的字符串输入（如 code viewer 的自定义插入格式 `codeSelectionFormat`，默认 `@{path}{lines}` 即 `@文件名:行号` @-引用）都通过卡片右下角的齿轮按钮在**原生弹窗**中编辑（复选框行 / 文本输入行，输入行失焦或回车提交），父级卡片关闭时齿轮隐藏；**key 必须是宿主 PrefsSchema 的字段**（内置键：`autoOpenSubagent` / `agentTerminalTools` / `htmlViewerNoSandbox` / `htmlViewerDefaultUnsafe` / 字符串键 `codeSelectionFormat`），外部插件的自定义键会被 settings seam 丢弃。
...
- **注册时机**：dsh-dashboard 在 `apply()` 开头 `ctx.provide('dashboard', service)`，所以你的插件 `inject = ['dashboard']` 时，dsh-dashboard 已经就绪。
...
| **双 cordis 实例** | 外部插件解析不到 DSH monorepo 的 cordis augmentation；dsh-dashboard 自己重述了 `interface Context { dashboard: ... }`，你 `import type {}` 即拿到类型 |
| **ModuleLoader 不跨插件** | 运行时 `require()` 虽支持跨 bundle，但被构建门挡；所有交互走 `ctx.dashboard` 方法调用 |
| **host 半无此服务** | `ctx.dashboard` 只在 client 侧存在；host 半需要侧边栏数据走 `/sidebar/api/*` HTTP 路由 |
| **portal 限制** | 整面板 slot 由 ui-layout 独占，外部 tab 只能进入 dsh-dashboard 的 portal 内部，无法全屏替换 |
| **id 冲突** | `registerTab` / `registerFileViewer` 对重复 id 抛错；建议用包前缀（`my-plugin:xxx`） |
| **i18n 跟随** | 侧边栏界面文案跟随 DSH 的 `ctx.locale`（`@deepseek-ai/dsh-client-locale`）：词典注册在 `dashboard` 命名空间，语言偏好（Host-backed `locale.preference`）与浏览器语言不一致时以 DSH 为准并实时切换；locale 服务缺失时回退浏览器语言。插件自身的 `t()`（`src/client/locales.ts`）由 `apply()` 挂接服务；消费插件**不要**依赖此内部函数——标题等字段传字符串或 `() => string` 即可（i18n 友好）。⚠️ 渲染 DSH 的 `MarkdownText` 时必须传 `codeLabels={{ copyLabel: t('copy'), copiedLabel: t('copied') }}`——该组件 cordis-free，漏传则代码块复制按钮回退硬编码中文 |
...
dsh-dashboard 自己的内置 tab 和 viewer 就是参考实现（"吃狗粮"）：

- **`src/client/builtins/`**：6 个内置 tab（explorer/git/subagent/terminal/editor/diff）+ 9 个内置 viewer（image/pdf/docx/xlsx/pptx/markdown/html/code/binary-download）的注册代码（tabs.tsx / viewers.tsx / index.ts）
...
- **`tests/builtins.spec.ts`**：内置注册清单断言（6 tab + 9 viewer + 声明式元数据）
