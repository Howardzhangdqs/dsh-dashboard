...
/**
 * The gzip-stored DATA assets the client may request (mirror of
 * src/client/file-icons.ts). Unlike chunks these are not scripts but JSON
 * members stored pre-compressed (assets/icons.json.gz, ~300KB for the full
 * Material Icon Theme); the route streams the .gz bytes verbatim with
 * `Content-Encoding: gzip`, so the BROWSER decompresses natively and the
 * wire cost is the stored size.
 */
export const ASSET_NAMES = ['icons'] as const
...
const etags = new Map<string, MemoEtag>()

/**
 * A bundle file's ETag (quoted hash), or undefined when the file is
 * missing. Hash is recomputed only when mtime/size changed (hashing a
 * multi-MB chunk per request is wasteful).
 */
async function etagOfFile(path: string, key: string): Promise<string | undefined> {
  try {
...
const etagOf = (name: ChunkName, chunkDir: string): Promise<string | undefined> =>
  etagOfFile(join(chunkDir, `client-${name}.js`), `${chunkDir}:${name}`)
...
export function createBundleRouteHandler(
  ...
  return async (req, res): Promise<void> => {
    ...
    // Data assets: /sidebar/bundle/<name>.json.gz — gzip members streamed
    // verbatim with Content-Encoding: gzip (browser-side decompression).
    const assetMatch = /^\/sidebar\/bundle\/([a-z0-9-]+)\.json\.gz$/.exec(pathname)
    const asset = assetMatch?.[1] as AssetName | undefined
    if (asset !== undefined) {
      if (!(ASSET_NAMES as readonly string[]).includes(asset)) {
        res.writeHead(404)
        res.end('not found')
        return
      }
      const path = join(chunkDir, `${asset}.json.gz`)
      const etag = await etagOfFile(path, `${chunkDir}:asset:${asset}`)
      if (etag === undefined) {
        res.writeHead(404)
        res.end('not found')
        return
