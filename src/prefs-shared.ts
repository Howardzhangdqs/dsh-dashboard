...
export const SIDEBAR_PREFS_NS = 'dsh-dashboard'
...
export interface SidebarPrefs {
   ...
   * `{start}`/`{end}` (line numbers, empty when unknown), `{lines}` (the
   * `:start` / `:start-end` suffix, empty when unknown), `{selection}`
   * (the selected text; empty when it exceeds the insertion limit) — are
   * substituted; every other character is kept verbatim and a literal
   * `\n` escape becomes a newline. An EMPTY stored value also resolves to
   * the default at build time (an empty insert is never useful). Lives
   * under the `code` viewer card in the Side card settings.
   */
  codeSelectionFormat: string
  /**
...
/**
 * The default "add to conversation" insert format: an @-reference carrying
 * the line span (`@相对路径:起止行`). Shared by the schema default, the
 * client fallback and the placeholder hint.
 */
export const SELECTION_FORMAT_DEFAULT = '@{path}{lines}'
