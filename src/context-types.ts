 ...
 * namespace registry under `dashboard`.
 */
...
declare module 'cordis' {
  interface Context {
     ...
     * dictionaries under the `dashboard` namespace. Client side only.
     */
     ...
     * {@link ./client/index.tsx}); undefined on the host side. Renamed with
     * the package (was `betterSidebar` — the alias below keeps external
     * consumers injecting the old name working).
     */
    dashboard: BetterSidebarService
    /** Compatibility alias for {@link dashboard} (the pre-rename service id). */
 ...
 * `ctx.dashboard` (and the `betterSidebar` alias).
 */
