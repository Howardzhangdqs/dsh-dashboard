/**
 * dsh-dashboard host half: the /sidebar JSON API (explorer listing, file
...
import { buildLlmStatsApi, type LlmStatsRoutes } from './llm-stats.ts'
import { readJsonBody, requireString, SidebarError, writeError, writeJson, writeOk } from './wire.ts'

export { Config }
export type { SidebarConfig, ResolvedSidebarConfig }
// Re-export the Context augmentation (declare module 'cordis') so consumers
// `import type {} from 'dsh-dashboard'` and gain `ctx.dashboard`.
...
function buildApi(
  ...
  // Model-call metrics: per-request tokens / cache hits / TTFT / duration /
  // tok-s, replayed from the session's own event log + the live append
  // mirror (the same zero-write pattern the jobs routes use).
  const llmStatsApi: LlmStatsRoutes = buildLlmStatsApi(ctx)
  return {
    'session.cwd': (payload) => {
    ...
    // Model-call metrics for the LLM stats tab: the session's recent model
    // requests with tokens, cache hits, TTFT, duration, and tok/s.
    'llm.stats': (payload) => llmStatsApi.stats(payload),
...
export function apply(ctx: Context, config?: SidebarConfig): void {
  ...
  }), 'dsh-dashboard: /sidebar/api routes')
  ...
  ctx.effect(() => registerBundleRoute(ctx, fence), 'dsh-dashboard: /sidebar/bundle chunk route')
  ...
  }), 'dsh-dashboard: /sidebar/file media route')
  ...
  }), 'dsh-dashboard: /sidebar/html preview route')
  ...
  }), 'dsh-dashboard: terminal WebSocket')
  ...
  }), 'dsh-dashboard: agent-terminals push WebSocket')
