 ...
 * bottom panel squeezes ONLY the center column (the agent output area)
 * while the right panel is open: it spans from the app shell's own left
 * sidebar to the right panel's left edge, so neither sidebar gives up any
 * position (the right panel keeps its full height). While the right panel
 * is COLLAPSED the bottom panel extends across the details (trajectory)
 * column to the viewport's right edge — the strip under the trajectory is
 * dead space otherwise — and layout.css lifts that column's content by the
 * same height so nothing is covered. The two panel toggles live in the
 * session header (SessionHeaderToggles, pinned right of the 对话/轨迹
 * switcher in the merged header row); the right panel's width drags from
 * its left edge, the bottom panel's height from its top edge, and the
 * shared corner drags both at once. The whole layout lives in the
 * per-session store, so switching conversations swaps the sidebar.
 *
...
import { iconIdForFile, MaterialIcon, useFileIcons } from './file-icons.tsx'
...
function TabContent(props: {
  ...
  const descriptor = ctx.dashboard?.getTab(tab.type)
...
export function Sidebar(props: { ctx: Context; store: SidebarStore }) {
  ...
  // Body attribute for the right panel's CLOSED state. layout.css keys off
  // it to lift the details (trajectory) column while the bottom panel is
  // extended across it (collapsed right panel ⇒ bottom panel reaches the
  // viewport's right edge). The panel toggles themselves live in the session
  // header (SessionHeaderToggles below) — no header padding is compensated.
    ...
    const connect = (): void => {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          store.reduce(s => ctx.dashboard?.isTabEnabled('terminal') === false
            ? s
      ...
      socket.onclose = () => {
        ...
        if (failures >= FAILURE_LIMIT) {
          console.error('[dsh-dashboard] agent-terminals connection failed; stopping reconnect loop', sessionId)
          return
        }
  ...
  useEffect(() => {
    ...
    if (ctx.dashboard?.isTabEnabled('subagent') === false) return
    ...
    ctx.dashboard?.openTab({ type: 'subagent', title: t('subagent') })
  ...
  useEffect(() => {
    ...
    if (ctx.dashboard?.isTabEnabled('terminal') === false) return
    ...
    ctx.dashboard?.openTab({ type: 'terminal' })
  ...
  const applyDrag = (width: number, height: number): void => {
    ...
    // drag write-only (no React re-render mid-drag). While the right panel
    // is COLLAPSED the bottom panel is extended to the viewport's right
    // edge (see the render formula) and stays there during a bottom-height
    // drag — the center-derived formula would pull it back under the
    // trajectory column mid-drag.
    bottomRef.current?.style.setProperty('right', state?.panelOpen === false
      ? '0px'
  ...
  // Material icon set for FILE tabs (see tabIconOf below) — like every
  // hook it lives ABOVE the no-session early return: the first render
  // (no session yet) and the post-setSession re-render must run the SAME
  // hooks (React #310 regression, pinned by tests/sidebar-hooks.spec.tsx).
  const fileIcons = useFileIcons()

  if (state === undefined || sessionId === undefined) {
    // No session: the panel toggles live in the session header (see
    // SessionHeaderToggles, registered into the header's utilities slot),
    // and a hidden header renders nothing — there is no per-session layout
    // to toggle yet.
    return null
  }
   ...
   */
  /**
   * The tab icon: FILE tabs (editor, and diff tabs carrying a worktree
   * path) render their Material glyph from the lazily-fetched icon set —
   * the same icons the explorer rows use, so an opened file is
   * recognizable in the tab strip. Everything else falls back to the
   * tab-type registry's descriptor icon (and the generic one until the
   * icon set arrives / on fetch failure). The icon-set HOOK itself lives
   * above the no-session early return (see `fileIcons` there).
   */
  const tabIconOf = (tab: SidebarTab): ReactNode => {
    const path = tab.path ?? (tab.diff?.kind === 'worktree' ? tab.diff.path : undefined)
    if (fileIcons !== null && path !== undefined) {
      const svg = fileIcons.icons[iconIdForFile(fileIcons, path)]
      if (svg !== undefined) return <MaterialIcon svg={svg} size={14} />
    }
    const descriptor = ctx.dashboard?.getTab(tab.type)
  ...
  return (
    <>
      {/*
        The panel toggles no longer live here: they are registered into the
        session header's utilities slot (SessionHeaderToggles below, wired in
        src/client/index.tsx) — pinned right of the 对话/轨迹 switcher in the
        merged header row. This tree renders only the panels themselves.
      */}
      {/*
      ...
      */}
      ...
      <div
        ...
        style={{
          ...
          // push is already baked into centerRect.right) — but only while
          // the right panel is OPEN. Collapsed, the bottom panel EXTENDS
          // across the details (trajectory) column to the viewport's right
          // edge: the freed strip is otherwise dead space under the
          // trajectory, and layout.css lifts that column's content by the
          // same bottom height (keyed on body[data-dsh-sidebar-collapsed])
          // so nothing is covered. Reopening snaps the edge back to the
          // conversation column and then tracks it per-frame as the grid
          // squeezes (centerRect re-measures each frame of the animation).
          right: state.panelOpen ? window.innerWidth - centerRect.right : 0,
          ...
          // fill — without it the corner looks cut off). No seam extended to
          // the viewport edge — there is nothing to seam against.
...
/**
 * The two panel toggles as a SESSION-HEADER citizen: registered into the
 * header's `conversation.session.header.utilities` slot (src/client/index.tsx).
 * In the merged header row (a 对话/轨迹 view switcher present) dashboard.css
 * pins the cluster to the row's RIGHT EDGE — right of the switcher pill, at
 * the native 28px icon-button size; in a single-view header it flows inline
 * in the utilities row, right of the "Session log" capsule. Either way it is
 * in-flow with the header's own controls rather than a fixed overlay on the
 * viewport corner. The header squeezes with the center column while the
 * right panel is open, so the buttons stay reachable in every layout state.
 *
 * Subscribes to the per-session store directly: the buttons track the
 * CURRENT session's panelOpen/bottomOpen and dispatch the same reducers the
 * old cluster did. Narrow viewports offer only the right-panel toggle (no
 * bottom panel exists there). While the store has no state for the session
 * (still seeding) the buttons render disabled — the header exists, so the
 * affordance keeps its place instead of popping in.
 */
export function SessionHeaderToggles(props: { store: SidebarStore }): ReactNode {
  const { store } = props
  const narrow = useNarrowViewport()
  const snapshot = useSyncExternalStore(
    useCallback((callback: () => void) => store.subscribe(callback), [store]),
  ...
  return (
    <div className={css.headerToggles}>
      {!narrow && (
        <Tooltip label={bottomOpen ? t('collapseBottomPanel') : t('expandBottomPanel')} side="bottom" delayMs={500}>
          <button
            type="button"
            className={css.toggleButton}
            disabled={state === undefined}
            aria-label={bottomOpen ? t('collapseBottomPanel') : t('expandBottomPanel')}
            onClick={() => { store.reduce(toggleBottomPanel) }}
          >
      ...
      <Tooltip label={panelOpen ? t('collapse') : t('expand')} side="bottom" delayMs={500}>
        <button
          type="button"
          className={css.toggleButton}
          disabled={state === undefined}
          aria-label={panelOpen ? t('collapse') : t('expand')}
