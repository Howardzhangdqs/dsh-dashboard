...
import { buildSelectionInsert, linesOfSelection, type SelectionLines } from './selection-payload.ts'
...
export function TextEditor(props: FileViewerProps) {
  ...
  /**
   * The popup's insert payload for one selection, built from the live
   * Side card format setting under the `code` viewer card (absent = the
   * `@{path}{lines}` default). Read at payload-build time — the store is
   * a stable singleton, so getPrefs() always returns the freshest commit.
   */
  const selectionInsertOf = (lines: SelectionLines | undefined, text: string): string =>
    buildSelectionInsert(path, scope.cwd, lines, text, props.store?.getPrefs().codeSelectionFormat ?? '')
  ...
  const handlePreviewMouseUp = (): void => {
    ...
    showPopup(
      selectionInsertOf(lines ?? undefined, text),
