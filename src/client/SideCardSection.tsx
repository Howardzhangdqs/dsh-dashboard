 ...
 * when a subagent appears", the Terminal page's model terminal tools, the
 * code viewer's selection-insert switch).
 *
...
/** Read one string pref by declarative key (missing = ''). */
function prefString(prefs: SidebarPrefs, key: string): string {
  const value = (prefs as unknown as Record<string, string>)[key]
  return typeof value === 'string' ? value : ''
}

/**
 * One declared string setting as a native text-input row. Local draft
 * state (the width row's recipe): commits on blur/Enter, only when the
 * draft actually changed. Re-seeds when the committed value changes (a
 * failed commit reverts the shown value; the remount keeps no stale
 * draft).
 */
function FeatureInputRow(props: {
  input: SidebarSettingInput
  value: string
  onCommit: (key: string, value: string) => void
}) {
  const { input, value, onCommit } = props
  const [draft, setDraft] = useState(value)
  const commit = (): void => {
    if (draft !== value) onCommit(input.key, draft)
  }
  const title = textOf(input.title)
  return (
    <label className={css.popupRowInput}>
      <span className={css.rowText}>
        <span className={css.title}>{title}</span>
        {textOf(input.desc) !== '' && <span className={css.desc}>{textOf(input.desc)}</span>}
      </span>
      <span className={css.control}>
        <Input
          type="text"
          className={css.formatInput}
          value={draft}
          placeholder={textOf(input.placeholder)}
          aria-label={title}
 ...
 * per declared toggle, then one native text-input row per declared string
 * input. Extracted so the rows are testable without opening the Modal (the
 * Modal portal renders only while open).
 */
export function FeatureSettingsRows(props: {
  toggles: readonly SidebarSettingToggle[]
  inputs?: readonly SidebarSettingInput[]
  prefs: SidebarPrefs
  onToggle: (toggle: SidebarSettingToggle, next: boolean) => void
  onCommitInput?: (key: string, value: string) => void
}) {
  const { toggles, inputs, prefs, onToggle, onCommitInput } = props
  return (
    <div className={css.popupRows}>
      ...
      {(inputs ?? []).map(input => (
        <FeatureInputRow
          key={`${input.key}:${prefString(prefs, input.key)}`}
          input={input}
          value={prefString(prefs, input.key)}
          onCommit={onCommitInput ?? (() => { /* no-op */ })}
        />
      ))}
    </div>
  )
...
export function SideCardSection({ store, service }: SideCardSectionProps) {
  ...
  /** Commit one declaratively-declared string input (a SidebarPrefs string field). */
  const onCommitSettingInput = (key: string, value: string): void => {
  ...
  return (
    <div className={css.section}>
          ...
          <Fragment key={tab.id}>
            {renderCard({
              ...
              onOpenSettings: prefs.tabsEnabled[tab.id] !== false
                && ((tab.settings?.toggles?.length ?? 0) > 0 || (tab.settings?.inputs?.length ?? 0) > 0)
                ? () => { setSettingsFor(tab) }
                : undefined,
            })}
          </Fragment>
      ...
      {/* 文件预览: one small card per registered file viewer. A viewer that
          declares `settings.toggles` carries the gear corner button opening
          its settings popup (e.g. the HTML sandbox escape hatch, the code
          viewer's selection-insert switch) — same as the tab cards above. */}
        ...
        {viewers.map(viewer => (
          <Fragment key={viewer.id}>
            {renderCard({
              ...
              // The settings gear only while the viewer is enabled: its
              // related settings are dormant while the viewer is off.
              onOpenSettings: prefs.viewersEnabled[viewer.id] !== false
                && ((viewer.settings?.toggles?.length ?? 0) > 0 || (viewer.settings?.inputs?.length ?? 0) > 0)
      ...
      {/* The secondary settings popup: a feature's declared related settings
          as native checkbox rows plus declared string inputs (Modal chrome
          is the app's own). Mounted only while a feature is open — the
          Modal primitive runs hooks unconditionally, so a closed-but-mounted
          Modal would break SSR (and the renderToString spec) under the test
          dual-react split. */}
      {settingsFor !== null && (
        <Modal
          open
        ...
        >
          <FeatureSettingsRows
            ...
            inputs={settingsFor.settings?.inputs ?? []}
            prefs={prefs}
            onToggle={onToggleSetting}
            onCommitInput={onCommitSettingInput}
          />
        </Modal>
      )}
