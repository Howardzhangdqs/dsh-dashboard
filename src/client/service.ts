 ...
 * published to the cordis context as `ctx.dashboard` (see
...
/** One declarative string setting of a tab/viewer, rendered as a text
 *  input row in the feature's settings popup (e.g. the code viewer's
 *  custom "add to conversation" insert format). */
export interface SidebarSettingInput {
  /** The SidebarPrefs string field this input reads and writes ('codeSelectionFormat'). */
  key: string
  /** Row title (i18n friendly: string or () => string). */
  title: string | (() => string)
  /** Row description (i18n friendly). */
  desc?: string | (() => string)
  /** The input's placeholder hint (i18n friendly). */
...
export interface SidebarSettingsDeclaration {
  ...
  /**
   * Extra string inputs rendered in the feature's settings popup (only
   * while the feature is enabled). Same key contract as `toggles` — the
   * field must exist on the host's PrefsSchema as a string (built-ins:
   * 'codeSelectionFormat'); unknown keys are dropped by the settings seam.
   */
...
/** The registry service published as `ctx.dashboard`. */
...
export function createBetterSidebarService(store: SidebarStore): BetterSidebarService {
  ...
  const registerTab = (descriptor: TabDescriptor): (() => void) => {
    if (tabs.has(descriptor.id)) {
      throw new Error(`[dsh-dashboard] tab type "${descriptor.id}" already registered`)
    }
  ...
  const registerFileViewer = (descriptor: FileViewerDescriptor): (() => void) => {
    if (viewers.has(descriptor.id)) {
      throw new Error(`[dsh-dashboard] file viewer "${descriptor.id}" already registered`)
    }
  ...
  const openTab = (seed: { type: string; title?: string; path?: string; diff?: SidebarTab['diff']; id?: string; url?: string }): void => {
    ...
    if (!isTabEnabled(seed.type)) {
      console.warn(`[dsh-dashboard] tab type "${seed.type}" is disabled in the side card settings`)
      return
    }
    store.reduce((state) => {
