...
export const zh = {
  ...
  settingsCodeSelectionFormatTitle: '自定义「添加到对话」插入格式',
  settingsCodeSelectionFormatDesc: '在代码或 Markdown 预览中选中文字后点击「添加到对话」，插入输入框的内容由这个模板生成。\n占位符：{path} 相对路径（如 src/app.ts）、{abspath} 绝对路径、{start}/{end} 起止行号、{lines} 行号后缀（如 :12 或 :12-15，行号未知时为空）、{selection} 选中的代码（超过 500 字符时替换为空）。\n模板中的其它文字原样保留；写 \\n 表示换行；未知占位符（如 {foo}）不会被替换。\n示例：\n· @{path}{lines} → @src/app.ts:12-15（默认格式：仅引用，不带代码）\n· 请看 {path} 第 {start}-{end} 行：\\n{selection} → 引用后换行，附上选中的代码\n· {abspath}{lines} → /home/me/proj/src/app.ts:12-15（绝对路径引用）\n· 帮我解释 {path} → 不带行号的纯引用\n留空恢复默认的 @引用格式。',
  settingsCodeSelectionFormatPlaceholder: '@{path}{lines}',
  subagent: '任务管理',
  openSubagent: '任务管理',
  ...
  llmInput: '输入',
  llmOutput: '输出',
  llmUnknownModel: '未知模型',
  llmCacheDetail: '读缓存 {read} · 写缓存 {write}',
  llmContextUsed: '上下文 {pct}%',
  llmEmpty: '还没有模型请求',
  llmError: '加载失败',
}
...
export const en: Record<keyof typeof zh, string> = {
  ...
  collapseComposer: 'Collapse input bar',
  expandComposer: 'Expand input bar',
  ...
  settingsCodeSelectionFormatTitle: 'Custom "Add to conversation" insert format',
  settingsCodeSelectionFormatDesc: 'Clicking "Add to conversation" on a selection in the code or Markdown preview inserts text built from this template.\nPlaceholders: {path} relative path (e.g. src/app.ts), {abspath} absolute path, {start}/{end} line span, {lines} line suffix (e.g. :12 or :12-15, empty when unknown), {selection} selected code (empty past 500 characters).\nAny other text in the template is kept verbatim; type \\n for a newline; unknown placeholders (e.g. {foo}) are not replaced.\nExamples:\n· @{path}{lines} → @src/app.ts:12-15 (the default: reference only, no code)\n· see {path} lines {start}-{end}:\\n{selection} → the reference, then the selected code on a new line\n· {abspath}{lines} → /home/me/proj/src/app.ts:12-15 (absolute-path reference)\n· explain {path} for me → plain reference without line numbers\nLeaving the box empty restores the default @-reference format.',
  settingsCodeSelectionFormatPlaceholder: '@{path}{lines}',
  subagent: 'Tasks',
  openSubagent: 'Tasks',
  ...
  llmStats: 'Model Calls',
  llmAgoHeader: 'Time',
  llmCalls: 'Calls',
  llmAvgTtft: 'Avg TTFT',
  llmTokens: 'Tokens',
  llmCacheHit: 'Cache hit',
  llmDuration: 'Duration',
  llmInput: 'Input',
  llmOutput: 'Output',
  llmUnknownModel: 'unknown model',
  llmCacheDetail: 'read {read} · write {write}',
  llmContextUsed: 'ctx {pct}%',
  llmEmpty: 'No model calls yet',
  llmError: 'Load failed',
}
 ...
 */
export const LOCALE_NS = 'dashboard'
