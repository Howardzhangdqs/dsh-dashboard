...
export const api = {
  ...
  /** The session's recent model API calls (tokens, cache, TTFT, tok/s)
   *  plus whole-log token totals (DSH's own status window). */
  llmStats: (scope: SessionScope, limit: number, signal?: AbortSignal) =>
    call<{ calls: unknown[]; totals: { calls: number; inputTokens: number; cacheReadTokens: number; cacheWriteTokens: number; outputTokens: number } }>('llm.stats', scopePayload(scope, { limit }), signal),
