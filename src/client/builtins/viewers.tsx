...
export function builtinViewers(): readonly FileViewerDescriptor[] {
  return [
    ...
    },
    {
      ...
      priority: -100,
      fetchStrategy: 'fsRead',
      // Declarative settings: the custom "add to conversation" insert
      // format (defaults to the `@path:lines` @-reference; the markdown
      // preview shares the popup).
      settings: {
        inputs: [{
          key: 'codeSelectionFormat',
          title: () => t('settingsCodeSelectionFormatTitle'),
          desc: () => t('settingsCodeSelectionFormatDesc'),
          placeholder: () => t('settingsCodeSelectionFormatPlaceholder'),
        }],
      },
      component: (props) => <LazyTextEditor {...props} />,
    },
