...
import { iconIdForFile, iconIdForFolder, MaterialIcon, useFileIcons } from './file-icons.tsx'
...
export function ExplorerView(props: {
  ...
  /** Material icon set — null until the lazy member arrives (generic icons meanwhile). */
  ...
  const renderLevel = (dir: string, depth: number): ReactNode => {
    ...
    return entries.map(entry => {
      if (entry.isDir) {
        ...
        return (
          <div key={entry.path}>
            ...
            >
              {icons !== null
                ? <MaterialIcon svg={icons.icons[iconIdForFolder(icons, entry.name, isOpen)] ?? ''} size={16} />
      ...
      return (
        ...
        >
          {icons !== null
            ? <MaterialIcon svg={icons.icons[iconIdForFile(icons, entry.name)] ?? ''} size={16} />
      ...
      <div className={css.explorerBody}>
        {root === undefined ? (
          ...
          <>
            ...
            >
              {icons !== null
                ? <MaterialIcon svg={icons.icons[icons.folderOpen] ?? ''} size={16} />
